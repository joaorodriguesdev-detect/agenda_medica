...
interface SidebarProps {
  ...
  adicionarAgendamento: (novo: any) => void; // Prop nova aqui!
}

export default function Sidebar({ setBuscaGlobal, adicionarAgendamento }: SidebarProps) {
  const [menuAtivo, setMenuAtivo] = useState("calendar");
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Estados dos inputs do modal
  ...
  const handleSalvar = () => {
    // Separa o nome do médico da especialidade baseado no hífen do select
    const partes = medicoEsp.split(" - ");
    const medico = partes[0] || "";
    const especialidade = partes[1] || "";

    // Monta o objeto perfeito para o Tabulator
    const novoAgendamento = {
      ...
      data: data || "2026-09-30",
      horario: horario || "00:00",
      status: "Pendente" // Status inicial fixo conforme você pediu!
    };

    // Envia pra tabela lá no page.tsx
    adicionarAgendamento(novoAgendamento);
    
    // Fecha o modal e limpa os campos para a próxima
          ...
          <div className="p-6 border-b border-[#F1F2F4] flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-[#F6F7F9] border border-[#E7E9EC] text-sm rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#0FA0EE] focus:bg-white transition-all placeholder:text-[#8A93A0] text-[#1C2530]"
              />
            </div>
          </div>
